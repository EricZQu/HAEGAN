...
class JTNNVAEDectest(nn.Module):

    def __init__(self, vocab, hidden_size, latent_size, depthT, depthG):
        super(JTNNVAEDectest, self).__init__()
        args = parser.parse_args()
        self.manifold = getattr(manifolds, args.manifold)()

        self.vocab = vocab
        self.hidden_size = hidden_size
        self.latent_size = latent_size = latent_size // 2 - 1 #Tree and Mol has two vectors
    ...
    def encode(self, jtenc_holder, mpn_holder):
        ...
        mol_vecs = self.manifold.logmap0(mol_vecs) ######
    ...
    def forward(self, x_batch, beta):
        ...
        # print(x_tree_vecs, x_tree_mess, x_mol_vecs)
        ...
        o = torch.zeros_like(z_tree_vecs)
        z_tree_vecs = torch.cat([o[:, 0:1], z_tree_vecs], dim=1)
        z_tree_vecs = self.manifold.expmap0(z_tree_vecs)
        ...
        print('word:', word_acc, word_loss.item(), '\t', 
              'topo:', topo_acc, topo_loss.item(), '\t',
              'assm:', assm_acc, assm_loss.item())
    ...
    def decode(self, x_tree_vecs, x_mol_vecs, prob_decode):
        ...
        o = torch.zeros_like(x_tree_vecs)
        x_tree_vecs = torch.cat([o[:, 0:1], x_tree_vecs], dim=1)
        x_tree_vecs = self.manifold.expmap0(x_tree_vecs)
