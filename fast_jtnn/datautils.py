...
class HyperbolicMolTreeFolder(object):
    ...
    def __iter__(self):
        for fn in self.data_files:
            ...
            dataset = HyperbolicMolTreeDataset(batches, self.vocab, self.assm)
            dataloader = DataLoader(dataset, batch_size=1, shuffle=False, collate_fn=lambda x:x[0])#, num_workers=self.num_workers)
...
class HyperbolicMolTreeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return HyperbolicTensorize(self.data[idx], self.vocab, assm=self.assm)

def HyperbolicTensorize(tree_batch, vocab, assm=True):
    set_batch_nodeID(tree_batch, vocab)
    smiles_batch = [tree.smiles for tree in tree_batch]
    jtenc_holder, adj = HyperbolicJTNNEncoder.tensorize(tree_batch)
    # _, mess_dict = JTNNEncoder.tensorize(tree_batch)
    jtenc_holder = jtenc_holder
    mpn_holder = HyperbolicMPN.tensorize(smiles_batch)
    ...
    jtmpn_holder = HyperbolicJTMPN.tensorize(cands, adj)
    batch_idx = torch.LongTensor(batch_idx)
...
class HyperbolicMPNTestMolTreeFolder(object):
    ...
    def __iter__(self):
        for fn in self.data_files:
            ...
            dataset = HyperbolicMPNTestMolTreeDataset(batches, self.vocab, self.assm)
            dataloader = DataLoader(dataset, batch_size=1, shuffle=False, collate_fn=lambda x:x[0])#, num_workers=self.num_workers)
...
class HyperbolicMPNTestMolTreeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return HyperbolicMPNTestTensorize(self.data[idx], self.vocab, assm=self.assm)

def HyperbolicMPNTestTensorize(tree_batch, vocab, assm=True):
    # set_batch_nodeID(tree_batch, vocab)
    # smiles_batch = [tree.smiles for tree in tree_batch]
    # jtenc_holder, adj = HyperbolicJTNNEncoder.tensorize(tree_batch)
    # # _, mess_dict = JTNNEncoder.tensorize(tree_batch)
